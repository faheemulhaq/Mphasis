...
def display_chat():
    if "history" in st.session_state and st.session_state.history:
        ...
        for i, (user, bot) in enumerate(st.session_state.history):
            with chat_container:
                st.markdown(f"""
                <div style="text-align: left; padding: 10px; background-color: #f1f1f1; border-radius: 10px; margin-bottom: 10px;">
                    <strong>Human:</strong> {user}
                </div>
                """, unsafe_allow_html=True)
                st.markdown(f"""
                <div style="text-align: right; padding: 10px; background-color: #d0e8f2; border-radius: 10px; margin-bottom: 10px;">
                    <strong>Dexter:</strong> {bot}
                </div>
                """, unsafe_allow_html=True)
...
def main():
    ...
    if 'history' not in st.session_state:
        st.session_state.history = []  # List to store chat history (user question, bot answer)
    ...
    if not st.session_state.authenticated:
        ...
        with st.sidebar.expander("Upload your documents here"):
        ...
        question = st.text_input("Ask Dexter a question:", key="question")
        if st.button("Send"):
            if question:
        ...
        st.markdown("---")
        if st.button("Click to Speak a Command"):
            voice_command = get_voice_command()
            if voice_command:
                with st.spinner("Thinking..."):
                    response = user_input(voice_command)
                    # Append the conversation to the chat history
                    st.session_state.history.append((voice_command, response))
                    text_to_speech(response)
                    # Refresh the chat display
                    st.experimental_rerun()

        # Mute/Unmute option
        if st.sidebar.checkbox("Text to Speech"):
